...
def test_create_income_with_category_ids_adds_distribution_rows(client):
  b = client.post("/budget/categories", json={"name": "Бета", "type": "beta", "base_amount": 200}).json()
  a = client.post("/budget/categories", json={"name": "Альфа", "type": "alpha", "base_amount": 100}).json()
...
def test_create_income_unknown_category_id(client):
  r = client.post("/budget/incomes", json={"payout_date": "2026-09-11", "amount": 1000, "category_ids": [99999]})
  assert r.status_code == 404
  assert r.json()["code"] == "NOT_FOUND"


def test_create_income_category_without_base_amount(client):
  cat = client.post("/budget/categories", json={"name": "Пустая база", "type": "empty_base"}).json()
  r = client.post(
    "/budget/incomes",
...
def test_item_create_uses_category_base_amount(client):
  income = client.post("/budget/incomes", json={"payout_date": "2026-03-01", "amount": 50000}).json()
  assert income["items"] == []
  cat = client.post("/budget/categories", json={"name": "Еда", "type": "food", "base_amount": 12000}).json()
...
def test_item_create_requires_amount_when_no_base(client):
  ...
  income = client.post("/budget/incomes", json={"payout_date": "2026-03-02", "amount": 50000}).json()
  r = client.post("/budget/items", json={"income_id": income["id"], "category_id": cat["id"]})
  assert r.status_code == 422
  assert r.json()["code"] == "HTTP_422"
...
def test_item_create_income_not_found(client):
  cat = client.post("/budget/categories", json={"name": "Еда", "type": "food2", "base_amount": 1000}).json()
  r = client.post("/budget/items", json={"income_id": 999999, "category_id": cat["id"]})
  assert r.status_code == 404


def test_item_patch_and_delete(client):
  cat = client.post("/budget/categories", json={"name": "ЖКХ", "type": "util", "base_amount": 5000}).json()
  income = client.post("/budget/incomes", json={"payout_date": "2026-07-20", "amount": 90000}).json()
  assert income["items"] == []
  created = client.post(
    "/budget/items",
    json={"income_id": income["id"], "category_id": cat["id"], "amount": 5000, "comment": "тест"},
  ).json()
...
def test_delete_income_cascades_items(client):
  cat = client.post("/budget/categories", json={"name": "Аренда", "type": "rent2", "base_amount": 10000}).json()
  income = client.post("/budget/incomes", json={"payout_date": "2026-08-01", "amount": 90000}).json()
