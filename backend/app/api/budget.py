...
def _sorted_items_for_income(
    items: list[BudgetItem], cat_by_id: dict[int, BudgetCategory]
) -> list[BudgetItemOut]:
    ordered = sorted(
        items,
        key=lambda i: (i.sort_order is None, i.sort_order if i.sort_order is not None else 0, i.id),
    )
    return [
        BudgetItemOut(
            id=i.id,
...
def get_distribution(
    year: int | None = Query(default=None, ge=2000, le=2100), db: Session = Depends(get_db)
):
    q = (
        select(BudgetIncome)
        .options(selectinload(BudgetIncome.items))
...
def create_income(body: BudgetIncomeCreate, db: Session = Depends(get_db)):
    ...
    for cid in category_ids:
        ...
        if category.base_amount is None:
            raise AppError(
                422,
                "HTTP_422",
                "У выбранной категории нет базовой суммы — укажите её в категории или добавьте строку вручную",
            )
    ...
    income_loaded = (
        db.execute(
            select(BudgetIncome)
            .where(BudgetIncome.id == income.id)
            .options(selectinload(BudgetIncome.items)),
        )
        .scalars()
        .one()
...
@router.patch("/incomes/{income_id}", response_model=BudgetIncomeOut)
def update_income(income_id: int, body: BudgetIncomeUpdate, db: Session = Depends(get_db)):
    income = db.get(BudgetIncome, income_id)
    if not income:
        raise AppError(404, "NOT_FOUND", "Выплата не найдена")
    if body.payout_date is not None:
        income.payout_date = body.payout_date
    ...
    income_loaded = (
        db.execute(
            select(BudgetIncome)
            .where(BudgetIncome.id == income_id)
            .options(selectinload(BudgetIncome.items)),
        )
        .scalars()
        .one()
...
def _next_item_sort_order(db: Session, income_id: int) -> int | None:
    m = db.execute(
        select(func.max(BudgetItem.sort_order)).where(BudgetItem.income_id == income_id)
    ).scalar_one()
    if m is None:
        return 0
    return int(m) + 1
...
def create_item(body: BudgetItemCreate, db: Session = Depends(get_db)):
    ...
    sort_order = (
        ...
        else _next_item_sort_order(db, body.income_id)
    )
    row = BudgetItem(
...
def create_category(body: BudgetCategoryCreate, db: Session = Depends(get_db)):
    if (
        body.type
        and db.execute(select(BudgetCategory).where(BudgetCategory.type == body.type)).first()
    ):
...
def update_category_name(
    category_id: int, body: BudgetCategoryUpdateName, db: Session = Depends(get_db)
):
    ...
    if db.execute(
        select(BudgetCategory).where(
            BudgetCategory.name == body.name, BudgetCategory.id != category_id
        )
    ).first():
        raise AppError(409, "DUPLICATE_NAME", "Категория с таким названием уже существует")
...
def update_category_base_amount(
    category_id: int, body: BudgetCategoryUpdateBaseAmount, db: Session = Depends(get_db)
):
    category = db.get(BudgetCategory, category_id)
    if not category:
        raise AppError(404, "NOT_FOUND", "Категория не найдена")
...
def update_category_color(
    category_id: int, body: BudgetCategoryUpdateColor, db: Session = Depends(get_db)
):
    category = db.get(BudgetCategory, category_id)
    if not category:
        raise AppError(404, "NOT_FOUND", "Категория не найдена")
